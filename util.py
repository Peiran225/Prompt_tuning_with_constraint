...
def get_paths(out_dir, model_name, method, task, do_zeroshot,
...
def prepend_task_tokens(tokenizer, inputs, n_prefix,logger):
    #create the tensors of the prompt we want to optimize by creating a dictionary of {"token_id": tensor, ...}, 
    #We create n_prefix tokens and asign then new token_ids, then we assign each id with a tensor
    ...
    def convert(inputs):
        ...
        new_input_ids=torch.cat([
                ...
                inputs["input_ids"][:,1:]], 1)#prepend each training data with a prompt, it will look like 
...
def prepend_task_tokens_with_specific_ids(inputs, task_token_ids,logger):
    
    def convert(inputs):
        n_train = inputs["input_ids"].shape[0] #the number of training data(texts to be classified)
        pd_ids = torch.tensor(task_token_ids)
        n_prefix = len(pd_ids)
        new_input_ids=torch.cat([
                pd_ids.repeat(n_train, 1),
                inputs["input_ids"][:,1:]], 1)#prepend each training data with a prompt, it will look like 
                                              #[[prompts ids, ids of the first training data ],
                                              # [promt ids, ids of the second training data], ....]  
